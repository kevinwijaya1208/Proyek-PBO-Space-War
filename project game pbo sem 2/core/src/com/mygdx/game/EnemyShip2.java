...
public class EnemyShip2 extends EnemyShip{
    ...
    public EnemyShip2(float xPosition, float yPosition, float width, float height, int health, float movementSpeed, int score, Texture shipTexture, Texture laserTexture) {
    ...
    @Override
    void randomMove() {
        // gerak diagonal random
        float arah = (float) Math.sin((random.nextDouble() * 2 * Math.PI));
        int randomDiagonal = random.nextInt(4);
        if (randomDiagonal == 0){
            directionVector.x = arah;
            directionVector.y = arah;
        }else if (randomDiagonal == 1){
            directionVector.x = arah;
            directionVector.y = -arah;
        } else if (randomDiagonal == 2){
            directionVector.x = -arah;
            directionVector.y = arah;
        }else {
            directionVector.x = -arah;
